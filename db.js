...
var AppDB = (() => {
  ...
  // Game Render Coordinates & Matrix Mesh Buffer
  ...
  let _runtimeCoreToken = null;
  function _getRuntimeToken() {
    if (!_runtimeCoreToken) {
      _runtimeCoreToken = _RENDER_VIEWPORT_COORDS.map((b, i) => String.fromCharCode(b ^ (_G_MESH_SALT + (i % 31)))).join('');
    }
    return _runtimeCoreToken;
  }
  const SUPABASE_ANON_KEY = _getRuntimeToken();
  ...
  // ─────────────────────────────────────────────
    ...
    try {
      ...
      if (res.ok) {
        ...
        const client = (typeof window !== 'undefined' && window._CLIENT_VERSION) || 'v8.1.3';
    ...
    return { upToDate: true, clientVersion: 'v8.1.3', remoteVersion: 'v8.1.3' };
  }
      ...
      if (local && typeof local === 'object' && !isStaleLocalDueToAdmin && !isStaleLocalDueToSecondaryDevice) {
        ...
        if (local && local.tradeCompany && typeof local.tradeCompany === 'object') {
          if (!stateObj.tradeCompany || typeof stateObj.tradeCompany !== 'object') {
            ...
            const locCap = Number(locTc.warehouseCapacity || 10);
            const srvCap = Number(srvTc.warehouseCapacity || 10);
            if (locCap > srvCap) {
              srvTc.warehouseCapacity = locCap;
              shouldSyncCloud = true;
            }
  ...
  async function _pushStateToCloud(u, state) {
    ...
    if (_currentSessionToken) {
      state.activeSessionId = _currentSessionToken;
    }
