...
const GameEngine = (() => {
        ...
        if (sec && sec.unlocked) {
          ...
          ['stage1', 'stage2', 'stage3', 'logistics'].forEach(stKey => {
            ...
            if (lvl > 1 && secDef.stages && secDef.stages[stKey]) {
              ...
              for (let i = 1; i < lvl; i++) {
                industryTotal += Math.floor(baseCost * Math.pow(1.65, i));
              }
            }
          });
  ...
  function ensureFarmState() {
    ...
    if (!f.dailyLiquidation || typeof f.dailyLiquidation !== 'object') {
      f.dailyLiquidation = { date: '', totalLiquidated: 0 };
    }
  ...
  function getFarmDailyLiquidationInfo(f) {
    ...
    const totalLiquidated = Math.max(0, Number(f.dailyLiquidation.totalLiquidated || 0));
    const remaining = Math.max(0, DAILY_FARM_LIQUIDATION_CAP - totalLiquidated);
    return {
      date: today,
      totalLiquidated,
  ...
  function getFarmState() {
    ...
    return {
      ...
      dailyLiquidation: getFarmDailyLiquidationInfo(f),
      storage: {
  ...
  return {
    ...
    plantFarmCrop,
    plantAllFarmPlots,
    plantAllPlots: plantAllFarmPlots,
    harvestFarmCrop,
    harvestAllFarmPlots,
    harvestAllPlots: harvestAllFarmPlots,
    upgradeFarmLand,
