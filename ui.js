...
// =========================================================================
// Global Anti-AutoClicker & Script Sentinel (درع الحماية الشامل ضد النقر الآلي)
// =========================================================================
(() => {
  if (typeof document === 'undefined') return;
  let lastUntrustedWarningTime = 0;

  ['click', 'mousedown', 'pointerdown'].forEach((eventType) => {
    document.addEventListener(eventType, (e) => {
      // If event is synthetic/untrusted (generated by script, console, bot, or Tampermonkey)
      if (e && e.isTrusted === false) {
        const target = e.target;
        // Whitelist legitimate browser controls (file download anchors, file pickers)
        if (target) {
          if (target.tagName === 'A' && target.hasAttribute('download')) return;
          if (target.tagName === 'INPUT' && target.type === 'file') return;
          if (target.id === 'btn-admin-update-money' || target.id === 'btn-admin-fraud-check') return;
        }

        // Intercept and completely abort synthetic event in capture phase
...
const UIController = (() => {
  console.log('[UI] Controller Loaded (v=108)');
  ...
  function setupStartMenu() {
    ...
    const closeSecurityCodesBtn = document.getElementById('btn-close-security-codes-modal');
    ...
    async function loadAndRenderSecurityCodes(forceRegenerate = false) {
      ...
      try {
        ...
        container.innerHTML = codes.map((c, idx) => {
          ...
          return `
            <div class="p-3 rounded-2xl border ${isUsed ? 'bg-slate-900/40 border-slate-800/80 opacity-50' : 'bg-slate-900/90 border-indigo-500/30 hover:border-indigo-500/60'} transition flex items-center justify-between shadow-md">
              ...
              ` : '<i class="fa-solid fa-check-double text-slate-600 text-sm ml-2"></i>'}
            </div>
          `;
        }).join('');
      } catch (err) {
    ...
    copySecurityCode = (code) => {
      ...
      if (navigator.clipboard && navigator.clipboard.writeText) {
        ...
        showToast('تم النسخ', `تم نسخ رمز الأمان: ${code}`, 'success');
      } else {
    ...
    openForgotPinModal = () => {
      ...
      const forgotCodeInput = document.getElementById('input-forgot-security-code');
  ...
  function setupEventListeners() {
    ...
    if (bjStandBtn) {
      bjStandBtn.addEventListener('click', executeBjStand);
    }
    ...
    const HELP_CONTENT = {
      'panel-admin': {
        ...
        desc: `محطة القيادة والمراقبة المالية الشاملة لمدير السيرفر (الآدمن):
        <br>• <strong>قمرة القيادة والمؤشرات</strong>: مراقبة حية لاستهلاك المعالج والذاكرة وسرعة الاتصال ومضاعف السيرفر.
        <br>• <strong>إدارة وتعديل اللاعبين</strong>: فحص بيانات الحساب، تعديل الأرصدة المصرفية، فحص الأصول والمشاريع، وسجل العقوبات والحظر.
        <br>• <strong>مكافحة الحسابات الوهمية (Anti-Cheat)</strong>: رصد شبكات التغذية والحسابات المشبوهة وفحص البصمات والأجهزة المشتركة.
        <br>• <strong>التحكم بالاقتصاد والأسواق</strong>: سياسات الضرائب، صناعة الأخبار الاقتصادية، إدارة المزادات الحية، وتوليد أكواد المكافآت والهدايا.`
      },
      'panel-dashboard': {
        title: 'لوحة التحكم والعمل اليومي (Dashboard)',
        desc: `محطة قيادتك المالية والتحكم اليومي:
        <br>• <strong>نوبة العمل العادية</strong>: تمنحك الراتب الأساسي لمهنتك الحالية ونقاط خبرة (XP) تؤهلك للترقية.
        <br>• <strong>النوبة الإضافية (Overtime)</strong>: تمنحك <strong>2.5 ضعف الراتب + 3 أضعاف الخبرة</strong> لتسريع مسيرتك المهنية.
        <br>• <strong>ترخيص الإدارة الذاتية (12-Hour AFK)</strong>: تفعيله يتيح لشركاتك حصد وضخ الأرباح تلقائياً في حسابك البنكي أثناء خروجك لمدة تصل إلى 12 ساعة.
        <br>• <strong>نُقطة الشات الحية (Money Drop)</strong>: راقب الشات العام! يمكن للمعلمين واللاعبين إلقاء أكياس نقود حية؛ التقط نصيبك فوراً (بحد أقصى 5 أكياس يومياً).
        <br>• <strong>المهام وصندوق المكافأة اليومية</strong>: أكمل مهامك لفتح الصندوق الذهبي اليومي والحصول على مكافآت ضخمة.`
      },
      'panel-careers': {
      ...
      'panel-business': {
        ...
        desc: `ماكينة التدفق النقدي والأرباح المستمرة كل ثانية:
        <br>• <strong>10 قطاعات استثمارية</strong>: من عربة القهوة والمطاعم، إلى مصافي البترول وشركات استكشاف الفضاء.
        <br>• <strong>التسعير الذكي</strong>: اضبط سعر منتجاتك بتوازن؛ السعر العالي يقلل الإقبال، والسعر المتوازن يحقق أعلى صافي ربح تراكمي.
        <br>• <strong>العمالة وخطوط الإنتاج</strong>: وظف عمالاً لرفع الطاقة الإنتاجية ومضاعفة العوائد المالية المتدفقة.
        <br>• <strong>توزيعات أرباح الشركاء</strong>: إذا كان لديك شركاء، تُحسب وتُجمع أرباحهم تلقائياً بدقة، وتُرسل لحساباتهم البنكية في إشعار موحد دون أي نقصان.`
      },
      'panel-bank': {
        title: 'البنك المركزي والتحويلات والقروض',
        desc: `حصنك المالي الآمن ومنظومة النمو المصرفي:
        <br>• <strong>فائدة الودائع المركبة</strong>: ينمو رصيدك البنكي تلقائياً كل دقيقة بفائدة مركبة مضمونة (+5% إضافية عند قيادة رولز رويس Phantom).
        <br>• <strong>التحويلات البنكية المباشرة</strong>: أرسل الأموال فورياً لأي لاعب بالسيرفر عبر رقم حسابه أو اسمه بسهولة وأمان تام.
        <br>• <strong>القروض والتمويل</strong>: اقترض سيولة فورية للتوسع وشراء المشاريع، وسدد الأقساط بانتظام لتجنب عقوبات وغرامات السجن المالي.`
      },
      'panel-investments': {
        title: 'الصناديق الاستثمارية ورأس المال',
        desc: `أوعية ادخارية مصرفية استراتيجية ذات عوائد مجزية ومضمونة:
        <br>• <strong>5 باقات استثمارية</strong>: تبدأ من الوديعة السريعة (ساعة واحدة بعائد +4%) وحتى الصندوق الإمبراطوري الماسي (36 ساعة بعائد +80%).
        <br>• <strong>تجميد رأس المال</strong>: يُقفل أصل المبلغ طوال مدة الاستثمار، ومع اكتمال المؤقت يودع الأصل مع الأرباح في حسابك البنكي فوراً.
        <br>• <strong>الضوابط اليومية</strong>: يسمح بتشغيل صندوقين نشطين كحد أقصى في نفس الوقت (2/2)، وإجمالي 5 استثمارات يومياً تتجدد 12 منتصف الليل.`
      },
      'panel-assets': {
        title: 'الأصول العقارية وأسطول السيارات',
        desc: `تجميد الأرباح في أصول حقيقية فاخرة:
        <br>• <strong>العقارات الاستثمارية</strong>: اشترِ الفلل، المكاتب، ناطحات السحاب، الجزر الخاصة، والمحطة المدارية الفضائية لجني إيجارات مستمرة كل ثانية.
        <br>• <strong>أسطول السيارات الفارهة</strong>: امتلك السيارات الملكية واستفد من خصائصها الحصرية أو قم بتأجيرها لجني أرباح دورية.
        <br>• <strong>مضاعفة صافي الثروة</strong>: تمثل العقارات والأصول ركيزة أساسية لرفع صافي ثروتك (Net Worth) والتصدر في الترتيب العام.`
      },
      'panel-stocks': {
        title: 'البورصة وأسواق المال والمضاربة',
        desc: `التداول الحي في سوق الأسهم والسلع الاستراتيجية:
        <br>• <strong>8 أسهم وأصول مالية</strong>: CIB، الشرقية للدخان، المصرية للاتصالات، فوري، الذهب، البيتكوين، والذكاء الاصطناعي.
        <br>• <strong>شريط الأخبار الاقتصادية</strong>: راقب الأخبار العاجلة الحية؛ فالأحداث السياسية والتجارية ترفع الأسهم أو تهبط بها في لحظات.
      ...
      'panel-store': {
        ...
        desc: `المعدات النادرة ومضاعفات القوة والإنتاج:
        <br>• <strong>القلم الذهبي</strong>: يمنحك +50% زيادة في خبرة العمل XP لتسريع الترقيات.
        <br>• <strong>المحامي الدولي</strong>: حماية قانونية وتخفيض لمخاطر الشرطة في السوق السوداء.
        <br>• <strong>معالج الكوانتم</strong>: يرفع كفاءة وأرباح كافة شركاتك بنسبة إضافية +50%.
        <br>• <strong>البطاقة الماسية وتصريح VIP</strong>: تعظيم فوائد البنك، تخفيض الضرائب، ومضاعفة أرباح الكازينو.`
      },
      'panel-trade': {
        title: 'شركة الاستيراد والتصدير الدولية والمستودع',
        desc: `التجارة العالمية والخدمات اللوجستية البحرية والجوية:
        <br>• <strong>الاستيراد</strong>: تعاقد على استيراد بضائع عالمية بأسعار الجملة وانتظر وصول الشحنة بمؤقتات زمنية واقعية.
        <br>• <strong>سعة المستودع وحصة التصدير</strong>: سعة مستودعك الجمركي تمثل حصتك اليومية للتصدير (Export Quota) وتتجدد يومياً في منتصف الليل.
        <br>• <strong>كوتة السلع الفاخرة</strong>: للسلع الاستراتيجية الثقيلة (التوربينات، رقائق الذكاء الاصطناعي، الذهب) حصص يومية مدروسة لمنع الاحتكار.
        <br>• <strong>مجلس المشترين والأسواق</strong>: اختر أفضل مشترٍ دولي يقدم أعلى سعر واستلم أرباح الشحنة فور وصولها.`
      },
      'panel-industry': {
      ...
      'panel-blackmarket': {
        title: 'السوق السوداء وعالم الظلال والتهريب',
        desc: `الصفقات الممنوعة وغسيل الأموال العالي المخاطر:
        <br>• صفقات سرية كبرى لتهريب البضائع والماس والآثار تدر أرباحاً خيالية كاش قذر (Dirty Cash).
        <br>• <strong>المداهمات الأمنية الفجائية</strong>: إذا تخطى كاشك القذر 100K، هناك فرصة مداهمة فجائية؛ اختر دفع الرشوة أو المقاومة (الفشل يترتب عليه مصادرة 50% وسجن 10 دقائق).
        <br>• <strong>غسيل الأموال الآلي</strong>: الشركات المرخصة تغسل أموالك القذرة تلقائياً وتحولها لرصيد بنكي نظيف بنسبة 25%.`
      },
      'panel-casino': {
        title: 'كازينو التسلية وألعاب المجازفة الملكية',
        desc: `ألعاب الحظ والمخاطرة الممتعة:
        <br>• <strong>7 ألعاب مميزة</strong>: الصاروخ (Crash)، السلوتس الملكية، الروليت، النرد، ولعبة <strong>بلاك جاك الصحراء (21)</strong>.
        <br>• <strong>لعبة الـ 21</strong>: اهزم الموزع للربح بـ 2x، وحقق 2.5x عند الحصول على Blackjack مباشرة.
        <br>• <strong>بطاقة VIP Casino Pass</strong>: تمنحك 2.6x أرباح Blackjack وتجعل التعادل (Push) فوزاً مؤكداً لصالحك!`
      },
      'panel-leaderboard': {
        title: 'عرش الأثرياء وتوب السيرفر (Leaderboard)',
        desc: `قاعة الشرف والتصنيف المباشر لجميع اللاعبين:
        <br>• الترتيب يعتمد بدقة على <strong>صافي الثروة الكلية (Net Worth)</strong> شاملة الكاش والبنك وقيمة الشركات والأصول والأسهم.
        <br>• يتوج أصحاب المراكز الثلاثة الأولى بالتاج الذهبي والرموز الملكية أمام كل رواد السيرفر.`
      }
    };
  ...
  const CHAT_FRAMES_CATALOG = [
    {
      id: 'none',
      name: 'المظهر الافتراضي (بدون إطار)',
      badge: 'كلاسيكي',
      badgeClass: 'bg-slate-800 text-slate-400 border-slate-700',
      description: 'المظهر الكلاسيكي العادي لفقاعة الرسالة بدون أي توهج أو أطر إضافية.',
      previewBubbleClass: 'bg-slate-800/90 border border-slate-700/80 text-slate-200',
      previewTag: '',
      senderGlowClass: 'text-yellow-400 font-bold',
      isFree: true
    },
    {
      id: 'crimson_flame',
  ...
  function getPlayerUnlockedChatGlows() {
    const st = (typeof GameEngine !== 'undefined' && GameEngine.state) ? GameEngine.state : {};
    const unlocked = new Set(Array.isArray(st.unlockedChatGlows) ? st.unlockedChatGlows : []);

    // Always include 'none'
    unlocked.add('none');

    // Currently active glow
    if (st.chatGlow && st.chatGlow !== 'none') {
      unlocked.add(st.chatGlow);
    }
    if (st.hasChatGlow) {
    ...
    // Ensure array is synced back on state
    if (GameEngine.state) {
      GameEngine.state.unlockedChatGlows = Array.from(unlocked);
    }
    return Array.from(unlocked);
  }

  function updateCurrentChatFrameBadge() {
    const labelEl = document.getElementById('chat-current-frame-label');
    if (!labelEl) return;
    const st = (typeof GameEngine !== 'undefined' && GameEngine.state) ? GameEngine.state : null;
    const activeGlow = (st && st.chatGlow) || 'none';
  ...
  function openChatFrameSelectorModal() {
    const modal = document.getElementById('chat-frame-selector-modal');
    const container = document.getElementById('chat-frames-list-container');
    if (!modal || !container) return;

    const st = (typeof GameEngine !== 'undefined' && GameEngine.state) ? GameEngine.state : {};
    const curUser = GameEngine.activeUsername || st.username || 'أنت';
    const activeGlow = st.chatGlow || 'none';
    const unlockedList = getPlayerUnlockedChatGlows();

    container.innerHTML = '';

    CHAT_FRAMES_CATALOG.forEach(frame => {
      const isCurrent = (activeGlow === frame.id) || (frame.id === 'none' && (!activeGlow || activeGlow === 'none'));
      const isOwned = frame.isFree || unlockedList.includes(frame.id);

      const card = document.createElement('div');
      card.className = `p-3.5 rounded-2xl bg-slate-950/80 border-2 ${isCurrent ? 'border-amber-400 shadow-lg shadow-amber-500/15' : (isOwned ? 'border-slate-800 hover:border-slate-700' : 'border-slate-800/60 opacity-80')} flex flex-col space-y-2.5 transition`;

      const statusBadgeHtml = isCurrent
        ? `<span class="px-2 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-400 border border-emerald-500/40 flex items-center gap-1 shrink-0"><i class="fa-solid fa-check"></i> مفعل حالياً</span>`
        : (isOwned
          ? `<span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-sky-500/20 text-sky-300 border border-sky-500/40 shrink-0">متاح لديك</span>`
          : `<span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-800 text-slate-500 border border-slate-700 flex items-center gap-1 shrink-0"><i class="fa-solid fa-lock text-[9px]"></i> غير مملوك</span>`);

      let actionBtnHtml = '';
      if (isCurrent) {
        actionBtnHtml = `<button type="button" disabled class="w-full py-2 bg-emerald-500/15 border border-emerald-500/30 text-emerald-400 rounded-xl text-xs font-black cursor-default flex items-center justify-center gap-1.5"><i class="fa-solid fa-check"></i> تم التفعيل (الإطار النشط)</button>`;
      } else if (isOwned) {
        actionBtnHtml = `<button type="button" onclick="window.UI.selectChatFrame('${frame.id}')" class="w-full py-2 bg-gradient-to-r from-amber-500 to-yellow-400 hover:from-amber-400 hover:to-yellow-300 text-slate-950 rounded-xl text-xs font-black transition flex items-center justify-center gap-1.5 shadow-md shadow-amber-500/20 active:scale-95 cursor-pointer"><i class="fa-solid fa-wand-magic-sparkles"></i> اختيار هذا الإطار</button>`;
      } else {
        actionBtnHtml = `<button type="button" disabled class="w-full py-2 bg-slate-800/40 border border-slate-800 text-slate-500 rounded-xl text-xs font-bold cursor-not-allowed flex items-center justify-center gap-1.5"><i class="fa-solid fa-lock text-xs"></i> غير متاح بحسابك (يتطلب باقة VIP)</button>`;
      }
  ...
  function closeChatFrameSelectorModal() {
    const modal = document.getElementById('chat-frame-selector-modal');
    if (modal) modal.classList.add('hidden');
  }

  function selectChatFrame(frameId) {
    if (!GameEngine.state) return;
    const curUser = GameEngine.activeUsername || GameEngine.state.username;
    const unlocked = getPlayerUnlockedChatGlows();

    if (frameId !== 'none' && !unlocked.includes(frameId)) {
      showToast('تنبيه', 'أنت لا تملك هذا الإطار بعد!', 'warning');
      return;
    }

    GameEngine.state.chatGlow = (frameId === 'none') ? 'none' : frameId;
    if (curUser && window._knownVipGlowPlayers) {
      window._knownVipGlowPlayers.set(curUser, (frameId === 'none' ? '' : frameId));
    }
    ...
    // Refresh chat messages display if visible
  ...
  function setupV2UIHandlers() {
    ...
    const btnOpenFramesModal = document.getElementById('btn-open-chat-frames-modal');
    const btnQuickChangeFrame = document.getElementById('btn-quick-change-frame');
    const btnCloseFramesModal = document.getElementById('btn-close-chat-frame-modal');
    const btnConfirmCloseFrames = document.getElementById('btn-confirm-close-chat-frame-modal');

    if (btnOpenFramesModal) {
      btnOpenFramesModal.addEventListener('click', openChatFrameSelectorModal);
    }
    if (btnQuickChangeFrame) {
      btnQuickChangeFrame.addEventListener('click', openChatFrameSelectorModal);
    }
    if (btnCloseFramesModal) {
      btnCloseFramesModal.addEventListener('click', closeChatFrameSelectorModal);
    }
    if (btnConfirmCloseFrames) {
      btnConfirmCloseFrames.addEventListener('click', closeChatFrameSelectorModal);
    }
    ...
    // Close chat drawer or frame modal on ESC key
    document.addEventListener('keydown', (e) => {
      if (e.key === 'Escape') {
        const frameModal = document.getElementById('chat-frame-selector-modal');
        if (frameModal && !frameModal.classList.contains('hidden')) {
    ...
    if (chatSendBtn && chatInput) {
      const doSendChat = async () => {
        ...
        try {
          ...
          let chatGlow = (GameEngine.state && GameEngine.state.chatGlow);
          if (chatGlow === 'none') {
            chatGlow = '';
  ...
  function renderChatMessages(msgs) {
    ...
    if (curUser && GameEngine.state) {
      if (GameEngine.state.chatGlow === 'none') {
        window._knownVipGlowPlayers.set(curUser, '');
      } else if (GameEngine.state.chatGlow) {
        window._knownVipGlowPlayers.set(curUser, GameEngine.state.chatGlow);
      } else if (GameEngine.state.hasChatGlow || GameEngine.state.activePackage === 'pkg_vip_crimson_flame' || GameEngine.state.activePackage === 'pkg_vip_chat_glow' || GameEngine.state.activePackage === 'pkg_vip_royal_ultimate') {
        const myGlow = (GameEngine.state.activePackage === 'pkg_vip_crimson_flame' ? 'crimson_flame' : (GameEngine.state.activePackage === 'pkg_vip_royal_ultimate' ? 'cyber_rainbow' : 'gold_neon'));
