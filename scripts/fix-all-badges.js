...
async function run() {
  for (const item of TOP10) {
    ...
    const patch = await fetch(`https://rasalmal.online/rest/v1/players?username=ilike.${encodeURIComponent(r.username)}`, {
      method: 'PATCH',
      headers: {
        'apikey': key,
    ...
    const resRows = await patch.json();
    console.log(`[${item.code}] ${r.username}:`, resRows[0]?.state?.seasonBadge);
  }

  console.log('All Top 10 badges verified and active!');
}
run();
